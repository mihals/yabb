//!!!!!!!!!! скопирован из Bouq1_2_2, нужно переделать!!!!!!!!!!!!!
// класс-букет из одного центра и шести лепестков -
// три лепестка одного цвета отличающихся цветом от центра и
// три лепестка другого цвета, отличного от цвета центра
export default class Bouq1_3_3
{
    constructor(scene,cX, cY)
    {
        ...
        this.grBqImg=this.scene.add.image(this.cX, this.cY, 'grBq1_2_2').setOrigin(0.5,0.5);
        this.prImg=this.scene.add.image(this.cX+130, this.cY-130, '25').setOrigin(0.5,0.5);
        //this.tweenPrImg=this.scene.add.image(this.cX+130, this.cY-130, '25').setOrigin(0.5,0.5);

        this.ballGroup=this.scene.add.group();
        for(var i=0;i<5;i++)
	    {
		    this.ballGroup.create(this.cX,this.cY,'empty');
        } 
    ...
    addBalloon(numClr)
    {
		...
		if(this.numBals==0)
		{
            this.ballGroup.getChildren()[4].setTexture(balImgName);
        ...
        if(this.numBals==1)
        {
            this.ballGroup.getChildren()[0].setTexture(balImgName);
        ...
        if(this.numBals==2)
        {
            ...
            {
                this.ballGroup.getChildren()[1].setTexture(balImgName);
            ...
            {
                this.ballGroup.getChildren()[2].setTexture(balImgName);
        ...
        if(this.numBals==3)
        {
            ...
            if(this.bouqArray[2]==-1)
            {
                ...
                {
                    this.ballGroup.getChildren()[3].setTexture(balImgName);
    ...
    clear()
    {
        ...
        {
            this.bouqArray[i] = -1;
            this.ballGroup.getChildren()[i].setTexture('empty');
        }
    ...
    createTween()
    {
        let twObj=this.scene.add.image(this.cX+130, this.cY-130, '25').setOrigin(0.5,0.5);
